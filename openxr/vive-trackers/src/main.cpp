...
#define b_qualify_xr(x) do {                                            \
    XrResult ret = x;                                               \
    if(XR_FAILED(ret)) {                                            \
        std::cout << "Openxr error: " << ret << std::endl; \
        return false;                                               \
    }                                                               \
} while(0)
...
{
    uint32_t unOutput;
    xrPathToString(instance, path, 0, &unOutput, nullptr);

    std::string result;
    result.resize(unOutput);

    xrPathToString(instance, path, result.size(), &unOutput, result.data());
...
int main()
{
    ...
    xr_get_proc(instance, xrEnumerateViveTrackerPathsHTCX);
    ...
    if (XrResult result = xrStringToPath(instance, "/interaction_profiles/htc/vive_tracker_htcx",
    ...
    if (XrResult result = xrStringToPath(instance, "/user/vive_tracker_htcx/role/chest/input/squeeze/click",
                                         &viveTrackerGrip))
    {
        std::cout << "String to path error: " << result << std::endl;
        return 1;
    ...
    if (XrResult result = xrStringToPath(instance, "/user/vive_tracker_htcx/role/chest/input/trigger/click",
                                         &viveTrackerTriggerClick))
    {
        std::cout << "String to path error: " << result << std::endl;
        return 1;
    }
    ...
    while (true)
    {
        ...
        while (pollResult == XR_SUCCESS)
        {
            ...
            case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED:
                {
                    const XrEventDataSessionStateChanged& sessionStateChanged = *reinterpret_cast<
        ...
        if (bSessionFocused)
        {
            ...
            uint32_t unOutput;
            if (XrResult result = xrEnumerateViveTrackerPathsHTCX(instance, 0, &unOutput, nullptr))
            {
                std::cout << "Enumerate error: " << result << std::endl;
                return 1;
            }

            std::vector<XrViveTrackerPathsHTCX> vPaths(unOutput);
            if (XrResult result = xrEnumerateViveTrackerPathsHTCX(instance, vPaths.size(), &unOutput, vPaths.data()))
            {
                std::cout << "Enumerate error: " << result << std::endl;
                return 1;
            }

            std::cout << "Num trackers: " << vPaths.size() << std::endl;

            for (XrViveTrackerPathsHTCX& path : vPaths)
            {
                std::cout << "Tracker " << PathToString(instance, path.persistentPath) << " " << PathToString(
                    instance, path.rolePath) << std::endl;
            }
