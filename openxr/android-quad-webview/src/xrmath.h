...
inline static void XrVector2f_Lerp( XrVector2f *result, const XrVector2f *a, const XrVector2f *b, const float fraction )
{
...
inline static float XrVector3f_Length2( const XrVector3f *v )
{
	return v->x * v->x + v->y * v->y + v->z * v->z;
}

inline static void XrVector4f_Scale( XrVector4f *result, const XrVector4f *a, const float scaleFactor )
{
	result->x = a->x * scaleFactor;
	result->y = a->y * scaleFactor;
	result->z = a->z * scaleFactor;
	result->w = a->w * scaleFactor;
}

inline static void XrVector2f_Add( XrVector2f *result, const XrVector2f *a, const XrVector2f *b )
{
	result->x = a->x + b->x;
	result->y = a->y + b->y;
}

inline static void XrVector2f_Sub( XrVector2f *result, const XrVector2f *a, const XrVector2f *b )
{
	result->x = a->x - b->x;
	result->y = a->y - b->y;
}
...
inline static void XrVector2f_Scale( XrVector2f *result, const XrVector2f *a, const float scaleFactor )
{
	result->x = a->x * scaleFactor;
	result->y = a->y * scaleFactor;
}

inline static float XrVector2f_Dot( const XrVector2f *a, const XrVector2f *b )
{
...
inline static void
XrQuaternionf_CreateFromAxisAngle( XrQuaternionf *result, const XrVector3f *axis, const float angleInRadians )
{
...
inline static void
XrQuaternionf_CreateFromAxisAngle( XrQuaternionf *result, const XrVector3f *axisAngle )
{
	double angle = XrVector3f_Length( axisAngle );
	if ( angle == 0 )
	{
		*result = {
				.x = 0.f,
	...
	else
	{
		...
		float vectorScale = sin( halfAngle ) / angle;
		*result = {
				.x = vectorScale * axisAngle->x,
				.y = vectorScale * axisAngle->y,
				.z = vectorScale * axisAngle->z,
				.w = cos( halfAngle ),
		};
	}
}
...
inline static void
XrQuaternionf_ToAxisAngle( XrVector3f *result, const XrQuaternionf *q )
{
...
inline static void
XrQuaternionf_Lerp( XrQuaternionf *result, const XrQuaternionf *a, const XrQuaternionf *b, const float fraction )
{
...
// Creates a translation matrix.
inline static void XrMatrix4x4f_SetTranslation( XrMatrix4x4f *result, const float x, const float y, const float z )
{
	result->m[ 12 ] = x;
	result->m[ 13 ] = y;
	result->m[ 14 ] = z;
...
inline static void XrQuaternionf_CreateFromMatrix4x4f( XrQuaternionf *result, const XrMatrix4x4f *matrix )
{
	float tr = matrix->m[ 0 ] + matrix->m[ 5 ] + matrix->m[ 10 ];

	if ( tr > 0 )
	{
		float s = 0.5f / sqrt( tr + 1.0f );

		result->w = 0.25f / s;
		result->x = (matrix->m[ 6 ] - matrix->m[ 9 ]) * s;
		result->y = (matrix->m[ 8 ] - matrix->m[ 2 ]) * s;
		result->z = (matrix->m[ 1 ] - matrix->m[ 4 ]) * s;

	}
	else if ( matrix->m[ 0 ] > matrix->m[ 5 ] && matrix->m[ 0 ] > matrix->m[ 10 ] )
	{
		float s = 0.5f / sqrt( 1.0f + matrix->m[ 0 ] - matrix->m[ 5 ] - matrix->m[ 10 ] );

		result->w = (matrix->m[ 6 ] - matrix->m[ 9 ]) * s;
		result->x = 0.25f / s;
		result->y = (matrix->m[ 4 ] + matrix->m[ 1 ]) * s;
		result->z = (matrix->m[ 8 ] + matrix->m[ 2 ]) * s;

	}
	else if ( matrix->m[ 5 ] > matrix->m[ 10 ] )
	{
		float s = 0.5f / sqrt( 1.0f + matrix->m[ 5 ] - matrix->m[ 0 ] - matrix->m[ 10 ] );

		result->w = (matrix->m[ 8 ] - matrix->m[ 2 ]) * s;
		result->x = (matrix->m[ 4 ] + matrix->m[ 1 ]) * s;
		result->y = 0.25f / s;
		result->z = (matrix->m[ 9 ] + matrix->m[ 6 ]) * s;

	}
	else
	{
		float s = 0.5f / sqrt( 1.0f + matrix->m[ 10 ] - matrix->m[ 0 ] - matrix->m[ 5 ] );

		result->w = (matrix->m[ 1 ] - matrix->m[ 4 ]) * s;
		result->x = (matrix->m[ 8 ] + matrix->m[ 2 ]) * s;
		result->y = (matrix->m[ 9 ] + matrix->m[ 6 ]) * s;
		result->z = 0.25f / s;
	}
...
inline static void
XrMatrix4x4f_CreateProjection( XrMatrix4x4f *result, GraphicsAPI graphicsApi, const float tanAngleLeft,
							   const float tanAngleRight, const float tanAngleUp, float const tanAngleDown,
							   const float nearZ, const float farZ )
{
...
inline static void
XrMatrix4x4f_CreateOffsetScaleForBounds( XrMatrix4x4f *result, const XrMatrix4x4f *matrix, const XrVector3f *mins,
										 const XrVector3f *maxs )
{
	...
	result->m[ 12 ] =
			matrix->m[ 12 ] + matrix->m[ 0 ] * offset.x + matrix->m[ 4 ] * offset.y + matrix->m[ 8 ] * offset.z;
	result->m[ 13 ] =
			matrix->m[ 13 ] + matrix->m[ 1 ] * offset.x + matrix->m[ 5 ] * offset.y + matrix->m[ 9 ] * offset.z;
	result->m[ 14 ] =
			matrix->m[ 14 ] + matrix->m[ 2 ] * offset.x + matrix->m[ 6 ] * offset.y + matrix->m[ 10 ] * offset.z;
	result->m[ 15 ] =
			matrix->m[ 15 ] + matrix->m[ 3 ] * offset.x + matrix->m[ 7 ] * offset.y + matrix->m[ 11 ] * offset.z;
}
...
inline static void
XrMatrix4x4f_TransformBounds( XrVector3f *resultMins, XrVector3f *resultMaxs, const XrMatrix4x4f *matrix,
							  const XrVector3f *mins, const XrVector3f *maxs )
{
...
inline static bool XrMatrix4x4f_CullBounds( const XrMatrix4x4f *mvp, const XrVector3f *mins, const XrVector3f *maxs )
{
	...
	for ( int i = 0; i < 8; i++ )
	{
		const XrVector4f corner = {
				(i & 1) != 0 ? maxs->x : mins->x, (i & 2) != 0 ? maxs->y : mins->y,
							   ...
							   const XrPosef *pose )
{
	...
	XrMatrix4x4f rotationMatrix;
	XrMatrix4x4f_CreateFromQuaternion( &rotationMatrix, &pose->orientation );

	XrMatrix4x4f translationMatrix;
	XrMatrix4x4f_CreateTranslation( &translationMatrix, pose->position.x, pose->position.y,
									pose->position.z );
...
inline static void XrQuaternionf_GetYawPitchRoll( const XrQuaternionf *quat, float &yaw, float &pitch, float &roll )
{
	yaw = atan2( 2.0 * (quat->y * quat->z + quat->w * quat->x),
				 quat->w * quat->w - quat->x * quat->x - quat->y * quat->y + quat->z * quat->z );
	pitch = asin( -2.0 * (quat->x * quat->z - quat->w * quat->y));
	roll = atan2( 2.0 * (quat->x * quat->y + quat->w * quat->z),
				  quat->w * quat->w + quat->x * quat->x - quat->y * quat->y - quat->z * quat->z );
}

inline float XrRadiansToDegrees( float *radians )
{
...
inline float XrDegreestoRadians( float fDegrees )
{
    return M_PI * fDegrees / 180.f;
}

static float
PerpendicularDistance( const XrVector2f &pt, const XrVector2f &start, const XrVector2f &end )
{
	XrVector2f d;
	XrVector2f_Sub( &d, &end, &start );

	//Normalise
	float mag = XrVector2f_Length( &d );
	if ( mag > 0.0 )
	{
		XrVector2f_Scale( &d, &d, 1.f / mag );
	}

	XrVector2f pv;
	XrVector2f_Sub( &pv, &pt, &start );

	//Get dot product (project pv onto normalized direction)
	float pvdot = XrVector2f_Dot( &d, &pv );

	//Scale line direction vector
	XrVector2f ds;
	XrVector2f_Scale( &ds, &d, pvdot );

	//Subtract this from pv
	XrVector2f a;
	XrVector2f_Sub( &a, &pv, &ds );
	return XrVector2f_Length( &a );
}

static size_t
RamerDouglasPeucker( const std::vector<XrVector2f> &vInPairs, float fltEpsilon, std::vector<XrVector2f> &vOutPairs )
{
	float dmax = 0;
	size_t index = 0;
	size_t end = vInPairs.size() - 1;
	size_t i;
	for ( i = 1; i < end; i++ )
	{
		float d = PerpendicularDistance( vInPairs[ i ], vInPairs[ 0 ], vInPairs[ end ] );
		if ( d > dmax )
		{
			index = i;
			dmax = d;
	...
	if ( dmax > fltEpsilon )
	{
		std::vector<XrVector2f> vecPointsA;
		std::vector<XrVector2f> vecPointsB;
		// Tricky, for the algorithm to work right you have to include the pivot
		// in both the first and second set.
		std::vector<XrVector2f> vecInA( vInPairs.begin(), vInPairs.begin() + index + 1 );
		std::vector<XrVector2f> vecInB( vInPairs.begin() + index, vInPairs.end());
		RamerDouglasPeucker( vecInA, fltEpsilon, vecPointsA );
		RamerDouglasPeucker( vecInB, fltEpsilon, vecPointsB );
		vOutPairs.insert( vOutPairs.end(), vecPointsA.begin(), vecPointsA.end() - 1 );
		vOutPairs.insert( vOutPairs.end(), vecPointsB.begin(), vecPointsB.end());
	}
	else
	{
		vOutPairs.push_back( vInPairs[ 0 ] );
		vOutPairs.push_back( vInPairs[ end ] );
	}
	return vOutPairs.size();
}

// Extract the FOV from a projection matrix.
inline static bool XrFovf_ExtractProjectionFov( XrFovf *result, const XrMatrix4x4f *matrix )
{
	XrMatrix4x4f matInv;
	XrMatrix4x4f_Invert( &matInv, matrix );

	XrVector4f p0;
	{
		XrVector4f vecTopLeftNear = {-1.f, 1.f, 0.f, 1.f};
		XrMatrix4x4f_TransformVector4f( &p0, &matInv, &vecTopLeftNear );
		XrVector4f_Scale( &p0, &p0, 1.f / p0.w );
	}

	XrVector4f p1;
	{
		XrVector4f vecBottomRightNear = {1.f, -1.f, 0.f, 1.f};
		XrMatrix4x4f_TransformVector4f( &p1, &matInv, &vecBottomRightNear );
		XrVector4f_Scale( &p1, &p1, 1.f / p1.w );
	}

	result->angleUp = atan( -p0.y / p0.z );
	result->angleDown = atan( -p1.y / p1.z );
	result->angleLeft = atan( -p0.x / p0.z );
	result->angleRight = atan( -p1.x / p1.z );

	return true;
}

inline static void
XrMatrix4x4f_CreateFromEulerXYZ( XrMatrix4x4f *result, const XrVector3f *rotation )
{
	const float cosRx = cosf( rotation->x );
	const float cosRy = cosf( rotation->y );
	const float cosRz = cosf( rotation->z );

	const float sinRx = sinf( rotation->x );
	const float sinRy = sinf( rotation->y );
	const float sinRz = sinf( rotation->z );

	result->m[ 0 ] = cosRz * cosRy;
	result->m[ 1 ] = sinRz * cosRy;
	result->m[ 2 ] = -sinRy;

	result->m[ 4 ] = -sinRz * cosRx + cosRz + sinRy * sinRx;
	result->m[ 5 ] = cosRz * cosRx + sinRz * sinRy * sinRx;
	result->m[ 6 ] = cosRy * sinRx;

	result->m[ 8 ] = sinRz * sinRx + cosRz * sinRy * cosRx;
	result->m[ 9 ] = -cosRz * sinRx + sinRz * sinRy * cosRx;
	result->m[ 10 ] = cosRy * cosRx;

	result->m[ 15 ] = 1.0f;
}

inline static void
XrQuaternionf_CreateFromEulerZXY( XrQuaternionf* result, const XrVector3f* euler)
{
